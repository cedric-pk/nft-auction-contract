...
/**
 * @title Kalao Auction Contract
 * @dev NFT Auction
 */
contract KalaoAuction{
	/*
	*  Storage
	*/
    // EIP 2981 supports
	...
	*/
    ...
    event AuctionEnded(address winner, uint256 amount, uint256 kalaoPart, uint256 royaltyAmount);
    event NFTClaimed();
    event BidClaimed();

	constructor(address _kalao,
                address _seller,
                address _nftAddr,
                uint256 _tokenId,
	...
	*/
	...
	function close() external {
		...
		if (highestBid > 0  && bids[highestBidder] > 0) {
			...
			// Transfer auction fee to Kalao
			uint256 kalaoPart = 5*(highestBid/ 200);
			payable(kalao).transfer(kalaoPart);
			...
			uint256 amount = highestBid - kalaoPart - royaltyAmount;
			payable(seller).transfer(amount);
    		emit AuctionEnded(highestBidder, amount, kalaoPart, royaltyAmount);
		}
	}
